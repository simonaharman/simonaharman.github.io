...
What got me thinking about all of this was my own approach to this challenge in our goal of creating an economic foundation for our own DeFi product. At Loki, we’re developing Blockswap, a cross-chain layer to add to the network of liquidity pools forming (Uniswap, Curve, Balancer, and so on). We’re offering a seamless method of swapping assets across chains with no wrapping, special wallet software, or complex integrations. You’ll be able to send Bitcoin directly from an exchange and get whatever token you want connected to Blockswap, Curve, or Uniswap.

In designing the cryptoeconomics for Blockswap, we have to think carefully about _who_ pays for the costs of keeping the system alive and healthy. For Blockswap, we have to consider 4 types of actors and how they interact with the system: Liquidity providers, infrastructure providers, regular swappers, and arbitrageurs.

The emerging field of DeFi wisdom will lead us to believe that the most important people to incentivize in this scheme are the liquidity providers. As swapping is only sensible if fees are relatively low, and more liquidity leads to lower fees, obviously we should be paying liquidity providers to keep their capital deployed on Blockswap.

Although this idea is nice, it’s also flawed. Nothing comes for free — and the only people who should be paying for Blockswap are those that use the service, not unsuspecting holders of a governance token. So how can we engineer the token economics to strike a good balance between the various demands of the different actors?

Liquidity providers under the Uniswap model are paid via slippage based fees. Impermanent loss is limited by these slippage based fees, and arbitrageurs are paid to ensure that the trading price on Uniswap is as close to the rest of the market as possible. If we change the fee structure to further mitigate impermanent loss, we’re paying for it by reducing arbitrage profits, which in turn increases the likelihood of price divergence on the liquidity pools. However, we only need to pay arbitrageurs the bare minimum amount to prevent price divergence — and since better liquidity means less slippage, arbitrageurs shouldn’t be much worse off if we do this.

The next demand we have to meet is around infrastructure and security costs. Uniswap has no native infrastructure and security, as Ethereum handles that, and the swappers are the ones who end up paying for those fees in the form of gas.
...
Forcing users to provide that collateral themselves severely limits real utility, so the cost of collateral has to be socialized in some sense. In Blockswap, we’re paying vault node operators to put up their own collateral to secure the vaults we use to create a trustless custodial service, and that payment comes in the form of coin emission.

How do we offset the cost of that emission? Once again, nothing comes for free, and fundamentally, we don’t want to wind up in the same position that COMP and other tokens are in — paying for things by creating new tokens with no practical mechanism of impeding or combatting inflation by stimulating demand for the currency. As such, we’re paying for this emission by requiring Blockswap users pay a fee in Loki — which is burned. This way, we can ensure that security will always be paid for automatically, and the potential for inflation that we create as a result of that is paid down by users of the platform.

So that just leaves the most critical element of it all: the regular user. We don’t have a monopoly on cross-chain swaps — the lion’s share of that role goes to Binance. It’s just horribly inconvenient — creating accounts, setting up security, 2FA, depositing, withdrawing, and so on. Blockswap offers convenience. It’s the only solution being proposed that allows for cross-chain swaps with no special software, no wrapping, no coin-side integration, accounts, or user-provided collateral.

Blockswap only needs to exist if there is a market for it, and if there is, users _will_ accept slightly higher fees for that experience. Further, if everyone — infrastructure providers, liquidity providers, and arbitrageurs — is incentivized to get more users performing swaps, this will drive Blockswap’s adoption.
...
In the end, we’re ensuring that if Blockswap is a valuable product, we’re stimulating demand for the native currency behind it and not simply printing tokens to pay people with poorly aligned incentives. The trend in DeFi at the moment doesn’t seem to care about this idea at all, regardless of whether or not I’m okay with it — this is how it is, and people are making money (for now). But in time, we will learn if these practices, market attitudes, and economic designs actually hold water.

MMT is an economic theory which provides a different way to approach macroeconomics. It has often been severely mischaracterized as saying you can just print money without consequence, and we see that mischaracterization being applied in governance tokens (whether intentionally or not). With Blockswap, we are pursuing a different strategy, aiming to develop a robust solution with sensible economics rather than value built solely on speculation.
